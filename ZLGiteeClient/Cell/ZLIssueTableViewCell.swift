...
class ZLIssueTableViewCell: UITableViewCell {
    ...
    private lazy var assitLabel: UILabel = {
        ...
        label2.font = .zlRegularFont(withSize: 12)
...
extension ZLIssueTableViewCell: ZLViewUpdatableWithViewData {
    ...
    func fillWithViewData(viewData: ZLIssueTableViewCellDelegate) {
        ...
        self.titleLabel.text = viewData.getIssueTitleStr()
        self.assitLabel.text = viewData.getIssueAssistStr()
        self.statusTag.text = viewData.isIssueClosed() ? ZLIconFont.IssueClose.rawValue : ZLIconFont.IssueOpen.rawValue
        self.statusTag.textColor = viewData.isIssueClosed() ? UIColor(named: "ZLIssueClosedColor") : UIColor(named: "ZLIssueOpenedColor")
        ...
        for (label, colorStr) in viewData.getIssueLabels() {
        ...
        self.labelStackView.snp.updateConstraints { (make) in
            make.height.equalTo(viewData.getIssueLabels().count == 0 ? 0 : 20)
        }
