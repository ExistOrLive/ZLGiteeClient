...
class ZLGiteeOAuthController: ZLBaseViewController {
    ...
    init(delegate: ZLGiteeOAuthControllerDelegate, serialNumber: String) {
    ...
    private func startPasswordOAuth() {
        // to-do
    }
            ...
            if let state = state,
               state == self.serialNumber {
                ...
                if let error_description = error_description {
                    self.delegate?.onAuthorizeCodeFail(error: ZLGiteeOAuthError.authorizeError(desc: error_description), serialNumber: self.serialNumber)
                    self.close()
                }
            }
        }
...
extension ZLGiteeOAuthController {
...
extension ZLGiteeOAuthController: WKUIDelegate,WKNavigationDelegate {
