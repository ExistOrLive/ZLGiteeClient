...
extension UIView {
    ...
    func showShareMenu(title: String,
        ...
        let alertAction5 = UIAlertAction.init(title: "拷贝链接", style: UIAlertAction.Style.default) { (_: UIAlertAction) in
            let pasteBoard = UIPasteboard.general
            pasteBoard.url = url
        }

        let alertAction1 = UIAlertAction.init(title: "在 Gitee 中打开", style: UIAlertAction.Style.default) { (_: UIAlertAction) in
...
//            ZLUIRouter.navigateVC(key: ZLUIRouter.WebContentController, params: ["requestURL": url])
        ...
        let alertAction2 = UIAlertAction.init(title: "在 Safari 浏览器中打开", style: UIAlertAction.Style.default) { (_: UIAlertAction) in
            UIApplication.shared.open(url, options: [:], completionHandler: {(_: Bool) in})
        }

        let alertAction3 = UIAlertAction.init(title: "分享", style: UIAlertAction.Style.default) { [weak self] (_: UIAlertAction) in
        ...
        let alertAction4 = UIAlertAction.init(title: "取消", style: UIAlertAction.Style.cancel, handler: nil)
